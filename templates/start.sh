...
echo "[..] Create restapi.secret"
cat > etc/restapi.secret << EOF
# Format: '<ID> <API url> <user> <password>'
default	http://localhost/api	judgehost	${judgehost_password}
EOF
echo "[ok] Created restapi.secret"
...
then
	...
	then
		...
		then
			...
			if grep -q TRUSTED_PROXIES webapp/.env
			then
				sed -i "s|TRUSTED_PROXIES=.*|TRUSTED_PROXIES=${DOCKER_GATEWAY_IP}|" webapp/.env
			else
				echo "TRUSTED_PROXIES=${DOCKER_GATEWAY_IP}" >> webapp/.env
			fi
		fi
	else
...
then
	...
	# fi
	echo ${domserver_admin_password} > etc/initial_admin_password.secret
	INSTALL=install
	...
	echo "[..] Add CDS user"
	/scripts/add_cds_user.php
	echo "[ok] Added CDS user"
else
